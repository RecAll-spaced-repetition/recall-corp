...
from app.schemas import TrainMarkAnswer, TrainCard, TrainCardExt, TrainLog, TrainLogCreate, AllStats, DayStat, UserOptParams, TrainPlan
...
class TrainService(BaseService):
    ...
    async def train_card(self, user_id: int, card_id: int, training: TrainMarkAnswer) -> TrainCardExt:
        ...
        return TrainCardExt.from_fsrs_card_with_scheduler(user_id, card, scheduler)
    ...
    async def get_collection_train_stats(self, user_id: int, collection_id: int) -> TrainPlan:
        ...
        # TODO: Вынести в конструктор TrainPlan
        scheduler = Scheduler() if not user.train_opt_params else Scheduler(user.train_opt_params)
        avg_curr_r = 0.0
        avg_year_r = 0.0
        avg_s = 0.0
