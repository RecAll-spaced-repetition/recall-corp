...
from app.core import get_settings, minio
from app.repositories import FileCardRepository, FileRepository, UserRepository
...
class StorageService(BaseService):
    ...
    async def upload_file(self, user_id: int, file: UploadFile) -> FileMeta:
        if file.size > get_settings().max_file_bytes_size:
            raise HTTPException(
                status_code=413,
                detail=f"Max file's size is {get_settings().max_file_mb_size} MB",
            )
        ...
        if not await user_repo.exists_user_with_id(user_id):
            raise HTTPException(
                status_code=401,
                detail="Only authorized users can upload files",
            )
        ...
        if (
            file_type is None
            or file_ext is None
            or file_type not in get_allowed_types()
            or file_ext not in get_allowed_exts()
        ):
        ...
        try:
            ...
            raise HTTPException(409, f"Failed to upload file: {str(e)}") from e

        file_repo = self.uow.get_repository(FileRepository)

        return await file_repo.create_one(
            FileCreate(
                owner_id=user_id,
                type=file_type,
                ext=file_ext,
    ...
    async def get_file_meta(self, file_id: int, user_id: int | None) -> FileMeta:
        file_repo = self.uow.get_repository(FileRepository)
        file = await file_repo.get_by_id(file_id, FileMeta)
    ...
    async def get_file_cards(self, file_id: int, user_id: int) -> list[int]:
        ...
        if file_meta.owner_id != user_id:
            raise HTTPException(
                status_code=401, detail="Only authorized owners can get file's cards"
            )

        file_card_repo = self.uow.get_repository(FileCardRepository)

        return await file_card_repo.get_file_cards_ids(file_id)
    ...
    async def delete_file(self, file_id: int, user_id: int):
        ...
        if not await user_repo.exists_user_with_id(user_id):
            raise HTTPException(
                status_code=401, detail="Only authorized owners can delete files"
            )
        ...
        try:
            ...
            raise HTTPException(404, f"Failed to delete file: {str(e)}") from e

        file_repo = self.uow.get_repository(FileRepository)

        await file_repo.delete_by_id(file_id)
