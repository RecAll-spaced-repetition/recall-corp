...
class CardService(BaseService):
    ...
    async def __create_connections(
        ...
        new_collections = await card_collection_repo.filter_owner_exist_collections(
            owner_id=owner_id,
    ...
    async def __update_connections(
        ...
        # TODO: Возможно стоит таки перенести это в репозиторий card_collection
        request_collections = set(
            await card_collection_repo.filter_owner_exist_collections(
                owner_id=owner_id,
                collections=collections,
            )
        )
        ...
        if delete_collections := list(old_collections.difference(request_collections)):
            await card_collection_repo.unset_card_collection_connections(
                card_id=card_id,
        ...
        if new_collections := list(request_collections.difference(old_collections)):
            await card_collection_repo.set_card_collection_connections(
                card_id=card_id,
    ...
    async def get_card(self, card_id: int, user_id: int | None) -> Card:
        card_repo = self.uow.get_repository(CardRepository)
        card = await card_repo.get_card_by_id(card_id, Card)
    ...
    async def get_card_collections(self, user_id: int, card_id: int) -> list[CollectionShort]:
        ...
        if not await card_repo.exists_card_with_owner(user_id, card_id):
            raise HTTPException(
                status_code=401,
                detail="Only authorized owners can get cards' collections",
            )
    ...
    async def get_card_files(self, card_id: int, user_id: int) -> list[FileMeta]:
        ...
        if card.owner_id != user_id:
            raise HTTPException(
                status_code=401,
                detail="Only authorized owners can get card's files",
            )

        file_card_repo = self.uow.get_repository(FileCardRepository)

        return await file_card_repo.get_card_files(card_id, FileMeta)
    ...
    async def update_user_card(
        ...
        updated_card = await card_repo.update_card_by_id(card_id, new_card.model_dump(), Card)
    ...
    async def delete_card(self, user_id: int, card_id: int) -> None:
        ...
        if not await card_repo.exists_card_with_owner(user_id, card_id):
            raise HTTPException(
                status_code=401,
                detail="Only authorized owners can delete cards",
            )
