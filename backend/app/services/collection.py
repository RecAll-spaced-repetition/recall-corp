    ...
    async def delete_collection(self, uow: UnitOfWork, user_id: int, collection_id: int) -> None:
        async with ((uow.begin())):
            ...
            if cards_without_collections := collection_cards.difference(cards_with_collections):
                await uow.get_repository(CardRepository).delete_cards(list(cards_without_collections))

    async def get_collection_training_cards(
            self, uow: UnitOfWork, user_id: int, collection_id: int
    ) -> list[int]:
        if not await uow.get_repository(UserRepository).exists_user_with_id(user_id):
            raise HTTPException(status_code=400)  ## ТУТ ДОЛЖНО БЫТЬ КАСТОМНОЕ ИСКЛЮЧЕНИЕ!
        collection_repo = uow.get_repository(CollectionRepository)
        ...
        return await uow.get_repository(TrainRecordRepository).get_collection_training_cards(user_id, cards)
