...
from app.schemas import TrainRecord
...
async def create_train_record(
        conn: AsyncConnection, card_id: int, user_id: int, train_mark: int, prev_progress: float
):
    repeat_date: datetime = datetime.now(timezone.utc)
    progress: float = compute_new_card_progress(prev_progress, train_mark)
    ...
    insert_query = insert(TrainRecordTable).values(
        card_id=card_id, user_id=user_id, repeat_date=repeat_date,
        next_repeat_date=next_repeat_date, progress=progress
