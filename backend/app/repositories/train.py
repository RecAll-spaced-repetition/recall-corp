...
class TrainCardRepository(BaseSQLAlchemyRepository):
    ...
    async def update_train_card(self, user_id: int, card: TrainCard) -> TrainCard:
        return await self.update_one(
            filter_expr=and_(
                self.table.c.user_id == user_id, self.table.c.card_id == card.card_id
            ),
            update_values=card.model_dump(exclude=["card_id", "user_id"]),
            output_schema=TrainCard,
        )

    async def get_user_train_cards(
        self,
        user_id: int,
        collection_cards: list[int] | None = None,
    ) -> list[TrainCard]:
        if collection_cards is not None and len(collection_cards) == 0:
            return []

        expr = self.table.c.user_id == user_id

        if collection_cards is not None:
            expr = and_(
                self.table.c.user_id == user_id,
    ...
    async def get_cards_waiting_train(
        self,
        user_id: int,
        collection_cards: list[int],
    ) -> tuple[list[int], datetime]:
        if len(collection_cards) == 0:
            return []

        (need_train_again, no_need_train_again, min_due) = (
            await self.connection.execute(
                select(
                    func.array_agg(aggregate_order_by(self.table.c.card_id, self.table.c.due))
                    .filter(self.table.c.due <= func.now())
                    .label("need_train_ids"),
                    func.array_agg(aggregate_order_by(self.table.c.card_id, self.table.c.due))
                    .filter(self.table.c.due > func.now())
                    .label("no_need_train_ids"),
        ...
        not_trained_cards = list(
            set(collection_cards) - set(need_train_again) - set(no_need_train_again)
        )

        if len(not_trained_cards) > 0:
            min_due = datetime.now(UTC)
...
class TrainLogRepository(BaseSQLAlchemyRepository):
    ...
    async def get_user_train_logs(self, user_id: int, chrono: bool = False) -> list[TrainLog]:
        ...
        expr = (
            select(self.table.c[*TrainLog.fields()])
    ...
    async def get_user_card_train_logs(self, card_id: int, user_id: int) -> list[TrainLog]:
        return await self.get_all_filtered(
            filter_expr=and_(self.table.c.card_id == card_id, self.table.c.user_id == user_id),
