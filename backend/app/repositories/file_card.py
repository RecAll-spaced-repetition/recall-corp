...
from app.models import CardTable, FileCardTable, FileTable
...
class FileCardRepository(BaseSQLAlchemyRepository):
    ...
    async def __set_card_files_connections(self, card_id: int, file_ids: list[int]) -> None:
        await self.connection.execute(
            insert(self.table),
            [{"card_id": card_id, "file_id": file_id} for file_id in file_ids],
        )

    async def __unset_card_files_connections(self, card_id: int, file_ids: list[int]) -> None:
        await self.delete(
            and_(
                self.table.c.card_id == card_id,
                self.table.c.file_id.in_(file_ids),
            )
        )

    async def __filter_owned_files(self, owner_id: int, file_ids: list[int]) -> list[int]:
        result = await self.connection.execute(
            select(self.file_table.c.id).where(
                and_(
    ...
    async def update_card_files_connections(
        ...
        """
        Обновляет связи карточки, удаляя не упомянутые в списке
        и добавляя связи, которых прежде не было.

        Обновляет публичность для всех отвязанных и привязанных
        файлов (новых и старых).
        """
