...
class TrainWhen(CollectionShort):
    when: TrainNever | TrainNow | TrainDue = Field(discriminator="type")

    @staticmethod
    def from_collection_short(
        collection: CollectionShort, when: TrainNever | TrainNow | TrainDue
    ):
        return TrainWhen(
            id=collection.id,
...
class CollectionStats(IdMixin):
    ...
    @staticmethod
    def from_cards_with_scheduler(
        id: int, all_cards_len: int, trained_cards: list[Card], scheduler: Scheduler
    ):
        if all_cards_len == 0:
...
class DayStat(CamelCaseBaseModel):
    ...
    def from_train_logs(all_logs: list[TrainLog]):
        ...
        logs: list[TrainLog] = []
        i = 0
        while i < total and train_date == all_logs[i].review_datetime.date():
            new_log = all_logs[i]
            cnt += 1
            avg_mark += new_log.rating
            duration = new_log.review_duration or 0
